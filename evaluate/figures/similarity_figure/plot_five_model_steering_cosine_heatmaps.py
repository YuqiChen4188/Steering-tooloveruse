...
DEFAULT_INPUT_SUMMARY = BASE_DIR / "steering_vector_pairwise_cosine.summary.json"
DEFAULT_OUTPUT_PREFIX = BASE_DIR / "five_model_steering_cosine_heatmaps"
...
DISPLAY_TOOL_LABELS = ["Code", "Search", "AskUser"]
PAIR_INDEX = {
    "code_search": (0, 1),
    "code_askuser": (0, 2),
    "search_askuser": (1, 2),
...
METRIC_LABELS = {
    "mean_layerwise_cosine": "Cosine similarity",
    "flattened_cosine": "Cosine similarity",
}
...
COLORBAR_AXES = [0.91, 0.20, 0.012, 0.58]
DEFAULT_CMAP = "white_orange"
ORANGE_HEX = "#F28E2B"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot 3x3 steering-vector cosine-similarity heatmaps for five models."
    )
    parser.add_argument(
        "--input-summary",
        type=Path,
        default=DEFAULT_INPUT_SUMMARY,
        help="Path to the summary JSON produced by analyze_steering_vector_pairwise_cosine.py.",
    )
    parser.add_argument(
        "--output-prefix",
        type=Path,
    ...
    parser.add_argument(
        ...
        default="mean_layerwise_cosine",
        help="Cosine metric to visualize in the off-diagonal cells.",
    )
    parser.add_argument(
        "--cmap",
        default=DEFAULT_CMAP,
        help="Matplotlib colormap name, or `white_orange` for the custom white-to-orange palette.",
    )
...
def build_model_matrix(model_summary: dict, metric: str) -> np.ndarray:
    matrix = np.eye(len(TOOL_LABELS), dtype=float)

    for pair_name, (row_idx, col_idx) in PAIR_INDEX.items():
        value = float(model_summary["pairwise_cosine"][pair_name][metric])
...
def build_colormap(cmap_name: str):
    if cmap_name == DEFAULT_CMAP:
        return colors.LinearSegmentedColormap.from_list(
            DEFAULT_CMAP,
            ["#FFFFFF", ORANGE_HEX],
        )
    return plt.get_cmap(cmap_name).copy()
...
def main() -> None:
    ...
    for panel_index, (ax, matrix, display_name) in enumerate(zip(axes, matrices, display_names)):
        ...
        ax.set_xticklabels(DISPLAY_TOOL_LABELS, fontsize=AXIS_LABEL_FONT_SIZE)
        if panel_index == 0:
            ax.set_yticklabels(DISPLAY_TOOL_LABELS, fontsize=AXIS_LABEL_FONT_SIZE)
        else:
            ax.set_yticklabels([])
    ...
    colorbar.set_label(METRIC_LABELS[args.metric], fontsize=COLORBAR_FONT_SIZE)
    colorbar.ax.tick_params(labelsize=COLORBAR_FONT_SIZE)

    fig.subplots_adjust(left=0.03, right=0.937, top=0.83, bottom=0.12, wspace=-0.2)
