#!/usr/bin/env python3
"""Run SteeringMark inference with heading-time vector orthogonalization."""
...
class TagTriggeredOrthogonalSteeringHookManager:
    """Project out the selected steering-vector direction on the last position."""
    ...
    def _make_hook(self, vector: torch.Tensor):
        def hook(_module, _inputs, output):
            ...
            vector_device = vector.to(device=hidden.device, dtype=hidden.dtype)
            denom = torch.dot(vector_device, vector_device)
            if not torch.isfinite(denom) or float(denom.item()) <= 0.0:
                return output

            hidden = hidden.clone()
            last_hidden = hidden[:, -1, :]
            coefficient = torch.matmul(last_hidden, vector_device) / denom
            projection = coefficient.unsqueeze(-1) * vector_device.unsqueeze(0)
            hidden[:, -1, :] = last_hidden - (projection * self.scale * self.strength)
...
def build_trigger_info(
    ...
    for model_layer_idx, vector_idx in selected_pairs:
        ...
        denom = torch.dot(steering_vector, steering_vector).item()
        if denom <= 0.0:
            coefficient = None
            ...
            coefficient_value = torch.dot(hidden_state, steering_vector).item() / denom
            projection = steering_vector * coefficient_value
            coefficient = float(coefficient_value)
            projection_norm = float(torch.linalg.vector_norm(projection).item())
            residual_norm = float(torch.linalg.vector_norm(hidden_state - projection).item())
            post_dot = float(torch.dot(hidden_state, steering_vector).item())
        ...
        per_layer.append(
            {
                ...
                "post_update_cosine_similarity": float(cosine),
                "signed_projection_coefficient": coefficient,
                "projection_norm": projection_norm,
                "residual_norm": residual_norm,
                "post_update_dot": post_dot,
            }
        )

    post_cosines = [abs(item["post_update_cosine_similarity"]) for item in per_layer]
    coefficients = [
        abs(item["signed_projection_coefficient"])
        for item in per_layer
        if item["signed_projection_coefficient"] is not None
    ]
    return {
        "context_tail_before_trigger": generated_text[-160:],
        "mean_abs_post_update_cosine": float(sum(post_cosines) / len(post_cosines)) if post_cosines else None,
        "max_abs_post_update_cosine": float(max(post_cosines)) if post_cosines else None,
        "mean_abs_projection_coefficient": float(sum(coefficients) / len(coefficients)) if coefficients else None,
        "per_layer_projection": per_layer,
    }


def summarize_token_diagnostics(token_diagnostics: list[dict[str, Any]]) -> dict[str, Any]:
    orthogonalized_steps = [
        item["generation_step"]
        for item in token_diagnostics
        if item.get("orthogonalization_active")
    ]
    trigger_infos = [item["trigger_info"] for item in token_diagnostics if item.get("trigger_info") is not None]
    post_cosines = [
        info["max_abs_post_update_cosine"]
        for info in trigger_infos
        if info.get("max_abs_post_update_cosine") is not None
    ]
    return {
        "num_generation_steps": len(token_diagnostics),
        "num_orthogonalized_steps": len(orthogonalized_steps),
        "first_orthogonalized_step": orthogonalized_steps[0] if orthogonalized_steps else None,
        "last_orthogonalized_step": orthogonalized_steps[-1] if orthogonalized_steps else None,
        ...
        "max_abs_post_update_cosine": max(post_cosines) if post_cosines else None,
    }


def greedy_generate_with_tag_triggered_orthogonalization_kv_cache(
    model: base.AutoModelForCausalLM,
    tokenizer: base.AutoTokenizer,
    prompt_ids: torch.Tensor,
    hook_manager: TagTriggeredOrthogonalSteeringHookManager,
    ...
    for generation_step in range(max_new_tokens):
        should_orthogonalize = base.has_open_heading_prefix(generated_text)
        hook_manager.set_scale(suppress_scale if should_orthogonalize else 0.0)
        with torch.inference_mode():
            outputs = model(
                ...
                use_cache=True,
                output_hidden_states=should_orthogonalize,
                return_dict=True,
            )
        ...
        trigger_info = None
        if should_orthogonalize and outputs.hidden_states is not None:
        ...
        diagnostics.append(
            {
                ...
                "orthogonalization_active": should_orthogonalize,
        ...
        for example in progress:
            ...
            while True:
                try:
                    ...
                    assistant_output, token_diagnostics = greedy_generate_with_tag_triggered_orthogonalization_kv_cache(
                        model=model,
                        tokenizer=tokenizer,
                    ...
                    steering_diagnostics.append(
                        {
                            "step": step_time,
                            "steering_mode": "vector_orthogonalization_heading_projection",
                            "domain": domain,
...
def initialize() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--steering_vector_dir",
        type=Path,
        default=Path("/data/yuqi/SteeringMark/steering_vector/Llama_3_8_vector_heading"),
        help="Directory containing step_mark_search/code/askuser payloads.",
    )
    parser.add_argument(
        "--steering_payload_name",
        type=str,
        default=None,
        help="Optional steering payload filename inside --steering_vector_dir. Overrides domain-specific payload selection.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Multiplier applied to the computed projection coefficient. 1.0 makes the updated state orthogonal to the steering vector.",
    )
    parser.add_argument(
        "--suppress_scale",
        type=float,
        default=1.0,
        help="Additional scale applied once a `###` heading prefix is active.",
    )
    ...
    parser.add_argument(
        ...
        default="summary",
        help="How much per-token orthogonalization trace to save. Default keeps only a compact summary.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
