...
def build_trigger_info(
    ...
    for model_layer_idx, vector_idx in selected_pairs:
        hidden_state = hidden_states[model_layer_idx + 1][0, -1, :].detach().float()
        steering_vector = steering_vectors[vector_idx].detach().to(device=hidden_state.device, dtype=hidden_state.dtype)
...
class TagTriggeredSuppressionHookManager:
    ...
    def __init__(
        ...
        self.vectors = steering_vectors.detach()
    ...
    def _make_hook(self, vector: torch.Tensor):
        vector_cache: dict[tuple[torch.device, torch.dtype], torch.Tensor] = {}
        ...
        def hook(_module, _inputs, output):
            ...
            if cached_vector is None:
                cached_vector = vector.to(device=hidden.device, dtype=hidden.dtype)
                vector_cache[cache_key] = cached_vector

            delta = cached_vector * self.scale * self.strength
            if hidden.shape[1] > 0:
                hidden = hidden.clone()
                hidden[:, -1, :] -= delta
