#!/usr/bin/env python3
"""Run SteeringMark tool-prompt inference with cosine-threshold-deviation-scaled heading suppression."""
...
def run_cosine_deviation_scaled_gate(
    hidden_states: tuple[torch.Tensor, ...],
    selected_pairs: list[tuple[int, int]],
    steering_vectors: torch.Tensor,
    layer_indices: list[int],
    threshold: float,
    max_suppress_scale: float,
    ...
    cosine_gap = (mean_cosine - threshold) if mean_cosine is not None else None
    positive_cosine_gap = max(cosine_gap, 0.0) if cosine_gap is not None else None
    cosine_headroom = max(1.0 - threshold, 1e-6)
    normalized_positive_gap = (
        min(positive_cosine_gap / cosine_headroom, 1.0) if positive_cosine_gap is not None else None
    )
    applied_suppress_scale = (
        max_suppress_scale * (normalized_positive_gap**scale_exponent)
        if normalized_positive_gap is not None and normalized_positive_gap > 0.0
        else 0.0
    )
    apply_steering = applied_suppress_scale > 0.0
    return {
        "apply_steering": bool(apply_steering),
        "threshold": float(threshold),
        "mean_selected_layer_cosine": float(mean_cosine) if mean_cosine is not None else None,
        "cosine_gap_from_threshold": float(cosine_gap) if cosine_gap is not None else None,
        "positive_cosine_gap": float(positive_cosine_gap) if positive_cosine_gap is not None else None,
        "normalized_positive_gap": float(normalized_positive_gap) if normalized_positive_gap is not None else None,
        "max_suppress_scale": float(max_suppress_scale),
        "scale_exponent": float(scale_exponent),
        "applied_suppress_scale": float(applied_suppress_scale),
...
def summarize_token_diagnostics(token_diagnostics: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "max_gate_applied_suppress_scale": (
            max(info["applied_suppress_scale"] for info in gate_infos)
            if gate_infos
            else None
        ),
...
def greedy_generate_with_cosine_deviation_scaled_suppression_kv_cache(
    ...
    for generation_step in range(max_new_tokens):
        ...
        if should_consider_heading:
            ...
            cosine_gate_info = run_cosine_deviation_scaled_gate(
                hidden_states=base_outputs.hidden_states,
                selected_pairs=selected_pairs,
                steering_vectors=steering_vectors,
                layer_indices=layer_indices,
                threshold=cosine_threshold,
                max_suppress_scale=max_suppress_scale,
                scale_exponent=scale_exponent,
            )
            steering_applied = cosine_gate_info["apply_steering"]
            if steering_applied:
                hook_manager.set_scale(cosine_gate_info["applied_suppress_scale"])
                with torch.inference_mode():
                    outputs = model(
                        input_ids=current_input_ids,
                        attention_mask=attention_mask,
...
def inference(args: argparse.Namespace) -> None:
    if args.suppress_scale < 0.0:
        raise ValueError("--suppress_scale must be non-negative.")
    if args.deviation_scale_power <= 0.0:
        raise ValueError("--deviation_scale_power must be > 0.")
        ...
        for example in progress:
            ...
            while True:
                try:
                    ...
                    assistant_output, token_diagnostics = greedy_generate_with_cosine_deviation_scaled_suppression_kv_cache(
                        model=model,
                        tokenizer=tokenizer,
                        prompt_ids=prompt_ids,
                        hook_manager=hook_manager,
                        max_suppress_scale=args.suppress_scale,
                        max_new_tokens=args.max_new_tokens,
                        selected_pairs=selected_pairs,
                        steering_vectors=steering_vectors,
                        layer_indices=layer_indices,
                        cosine_threshold=args.cosine_threshold,
                        scale_exponent=args.deviation_scale_power,
                    )
                    raw.append(assistant_output)
                    steering_diagnostics.append(
                        {
                            "step": step_time,
                            "gating_mode": "cosine_threshold_deviation_scaled_heading_suppression",
                            "domain": domain,
                            ...
                            "max_suppress_scale": args.suppress_scale,
                            "cosine_threshold": args.cosine_threshold,
                            "deviation_scale_power": args.deviation_scale_power,
...
def initialize() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Maximum suppress-only scale used when the mean cosine is far above --cosine_threshold.",
    )
    parser.add_argument(
        "--cosine_threshold",
        type=float,
        default=0.25,
        help="Start applying negative steering only when the mean cosine similarity at a `###` heading prefix exceeds this threshold.",
    )
    parser.add_argument(
        "--deviation_scale_power",
        type=float,
        default=1.0,
        help="Exponent applied to the normalized positive cosine-threshold gap before mapping it to --suppress_scale.",
    )
